...
CreateFixPartitionConstraintsTaskList(Oid relationId)
{
	...
	/* get a list of all check constraints that may need renaming */
...
/*
 * WorkerFixPartitionConstraintCommandList creates a list of command strings that will fix
										...
										List *checkConstraintList)
{
	...
	foreach_ptr(constraintName, checkConstraintList)
	{
		...
		appendStringInfo(shardQueryString,
						 "SELECT worker_fix_partition_constraints(%s::regclass, "
