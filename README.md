...
- **Distributed SQL query engine** routes and parallelizes SELECT, DML, and other operations on distributed tables across the cluster.
...
```
You can also set up a [multi-node Citus cluster with docker-compose]( https://docs.citusdata.com/en/stable/installation/single_machine_docker.html).
...
For more details, see our [documentation on how to set up a multi-node Citus cluster]( https://docs.citusdata.com/en/stable/installation/multi_machine.html) on various operating systems.
...
-- each node will contain a different subset of devices
...
-- get the last 3 events for devices whose name starts with laptop, parallelized across shards
